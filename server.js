...
const DB_FILE=process.env.DB_PATH||path.join(DATA_DIR,'atelier-v5.json');
...
function load(){if(!fs.existsSync(DB_FILE)){const d=clone(DEFAULT_DB);save(d);return d}try{const d=JSON.parse(fs.readFileSync(DB_FILE,'utf8'));return normalize(d)}catch{return clone(DEFAULT_DB)}}
...
function currentUser(req){const sid=parseCookies(req).atelier_v5_session;if(!sid)return null;const s=sessions.get(sid);if(!s||s.expires<Date.now()){sessions.delete(sid);return null}return db.users.find(u=>u.id===s.userId&&u.active!==false)||null}
...
async function route(req,res){
 ...
 if(p==='/health')return json(res,200,{ok:true,service:'atelier-natalia-huebra',version:'5.0.0'});
 ...
 if(p==='/api/auth/logout'&&req.method==='POST'){const sid=parseCookies(req).atelier_v5_session;if(sid)sessions.delete(sid);res.setHeader('Set-Cookie','atelier_v5_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0');const user=currentUser(req);if(user){audit(user,'logout','user',user.id);save()}return json(res,200,{ok:true})}
 ...
 if(p.startsWith('/api/admin')||p==='/api/dashboard'||p==='/api/audit'||p==='/api/settings'){
   ...
   if(p==='/api/settings'&&req.method==='PUT'){const b=await bodyJson(req);db.settings={...db.settings,...b};audit(user,'update','settings',null);save();return json(res,200,db.settings)}
   if(p==='/api/admin/me')return json(res,200,{ok:true,user:{id:user.id,email:user.email,role:user.role}});
   if(p==='/api/admin/logout'&&req.method==='POST'){return json(res,200,{ok:true})}
